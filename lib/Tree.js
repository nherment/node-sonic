...
function Tree(workflow) {
  ...
  var root
  if(!workflow.length || workflow.length > 0 && workflow[0].action === 'sc-fork') {
    root = new TreeNode({action: "sc-logger", message: "starting workflow"})
...
Tree.prototype.parse = function(node, workflow) {
  if(workflow && workflow.length > 0) {
    for(var i = 0 ; i < workflow.length ; i++) {
      ...
      // TODO: do not hardcode sc-fork like that
      if(options.action === 'sc-fork') {
